...
using Orleans;
using Orleans.SignalR;
using Orleans.SignalR.Tests.AspnetSignalR;
using Orleans.SignalR.Tests.Models;
using System;
...
namespace Orleans.SignalR.Tests
{
    public class OrleansHubLifetimeManagerTests : IClassFixture<OrleansFixture>
    {
        ...
        {
            using var manager1 = new OrleansHubLifetimeManager<MyHub>(new LoggerFactory().CreateLogger<OrleansHubLifetimeManager<MyHub>>(), _fixture.ClientProvider);
            using var manager2 = new OrleansHubLifetimeManager<MyHub>(new LoggerFactory().CreateLogger<OrleansHubLifetimeManager<MyHub>>(), _fixture.ClientProvider);
            ...
            await manager2.AddToGroupAsync(connection.ConnectionId, "easye2");
        ...
        {
            using var manager = new OrleansHubLifetimeManager<MyHub>(new LoggerFactory().CreateLogger<OrleansHubLifetimeManager<MyHub>>(), _fixture.ClientProvider);
            using var client = new TestClient();
            var connection = HubConnectionContextUtils.Create(client.Connection);
        ...
        public async Task DisconnectConnection_RemovesConnection_FromGroup()
        {
            ...
            await manager.OnConnectedAsync(connection);
            await manager.AddToGroupAsync(connection.ConnectionId, "drdre");

            await manager.OnDisconnectedAsync(connection);

            var grain = _fixture.ClientProvider.GetClient().GetGroupGrain("MyHub", "drdre");
        ...
        public async Task Hub_InterfaceMatchingNaming_Output()
        {
            ...
            using var manager = new OrleansHubLifetimeManager<DaHub>(new LoggerFactory().CreateLogger<OrleansHubLifetimeManager<DaHub>>(), _fixture.ClientProvider);
        ...
        {
            using var client1 = new TestClient();
            using var manager = new OrleansHubLifetimeManager<DaHubx>(new LoggerFactory().CreateLogger<OrleansHubLifetimeManager<DaHubx>>(), _fixture.ClientProvider);
        ...
        {
            using var client1 = new TestClient();
            using var manager = new OrleansHubLifetimeManager<DaHubUsingBase>(new LoggerFactory().CreateLogger<OrleansHubLifetimeManager<DaHubUsingBase>>(), _fixture.ClientProvider);
        ...
        public async Task InvokeAllAsync_ForSpecificHub_WithMultipleServers_WritesTo_AllConnections_Output()
        {
            using var manager1 = new OrleansHubLifetimeManager<MyHub>(new LoggerFactory().CreateLogger<OrleansHubLifetimeManager<MyHub>>(), _fixture.ClientProvider);
            using var manager2 = new OrleansHubLifetimeManager<MyHub>(new LoggerFactory().CreateLogger<OrleansHubLifetimeManager<MyHub>>(), _fixture.ClientProvider);
            using var manager3 = new OrleansHubLifetimeManager<DifferentHub>(new LoggerFactory().CreateLogger<OrleansHubLifetimeManager<DifferentHub>>(), _fixture.ClientProvider);
        ...
        {
            const string invalidConnection = "NotARealConnectionId";
